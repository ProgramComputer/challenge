...
import { Handle, Position, type Node as FlowNode, type NodeProps as FlowNodeProps } from "@xyflow/react";
...
export type AppServerNodeType = FlowNode<CustomNodeData, "app-server">;
export type ProxyNodeType = FlowNode<CustomNodeData, "proxy">;
export type ShardNodeType = FlowNode<CustomNodeData, "shard">;

export type AppNode = AppServerNodeType | ProxyNodeType | ShardNodeType;
...
export const AppServerNode: React.FC<NodeProps> = ({ data }) => {
  ...
  return (
    <div className="px-4 py-2 shadow-lg rounded-md bg-white border-2 border-green-500" style={{ width: NODE_WIDTH }}>
      ...
      <div 
        className={`bg-green-500 text-white p-4 rounded text-center text-2xl font-bold transition-transform duration-100 
          ${isPressed ? 'scale-95' : ''} 
          ${data.isInserting ? 'opacity-75' : ''}`}
      >
        insert
        <div className="text-2xl">row</div>
      </div>
...
export const ProxyNode: React.FC<NodeProps> = ({ data }) => {
  return (
...
export const ShardNode: React.FC<NodeProps> = ({ data }) => {
  return (
...
export const DataBox: React.FC<DataBoxProps> = ({ data }) => {
  return (
    <div 
